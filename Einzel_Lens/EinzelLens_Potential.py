...
class CartesianGrid:                                                            #基本構造
    ...
    def make_cylinder(self, v1, v2, z1, z2, z3, z4, z5, z6):
        ...
        for i in range(int(self.nz*(z3-self.zmin)/(self.zmax - self.zmin)), int(self.nz*(z4-self.zmin)/(self.zmax - self.zmin))):
            Z_new = np.vstack((Z_new, v1))

        for i in range(int(self.nz*(z4-self.zmin)/(self.zmax - self.zmin)), int(self.nz*(z5-self.zmin)/(self.zmax - self.zmin))):
            Z_new = np.vstack((Z_new, zeros))

        for i in range(int(self.nz*(z5-self.zmin)/(self.zmax - self.zmin)), int(self.nz*(z6-self.zmin)/(self.zmax - self.zmin))):
            Z_new = np.vstack((Z_new, v2))

        for i in range(int(self.nz*(z6-self.zmin)/(self.zmax - self.zmin)), int(self.nz*(self.zmax-self.zmin)/(self.zmax - self.zmin))-1):
            Z_new = np.vstack((Z_new, zeros))
...
def solve_eq():
    ...
    V_num = input("EinzelLens印加電圧：")
    mesh = CartesianGrid()
    
    # extrode boundary condition
    V_1 = mesh.set_boundary_circle(V_num, 19.65, 21.35)
    V_0 = mesh.set_boundary_circle(1e-100, 19.65, 21.35)

    electrode = mesh.make_cylinder(V_1, V_0, -45, -25, -20, 10, 15, 45)
