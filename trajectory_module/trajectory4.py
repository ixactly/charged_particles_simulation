...
class CartesianGrid:                                                            #基本構造
    ...
    def __init__(self, nx=150, ny=150, nz=150, xmin=-25, xmax=25, ymin=-25, ymax=25, zmin=195, zmax=405):
...
delta_y = (mesh.xmax - mesh.xmin)/((mesh.nx-1)*1000)
delta_z = (mesh.zmax - mesh.zmin)/((mesh.nz-1)*1000)
...
def calc_trajectory(itera, data, sample):
    ...
    for a in range(itera):
        ...
        for b in range(sample):                         #軌道計算
            ...
            if (-19.4 < y0 < -12) or (-11.0 < y0 < 11.0) or (12 < y0 < 19.4):
                vzlist[b].append(vz0)
                zlist[b].append(z0)
                ylist[b].append(y0)
                while mesh.zmin-0.3<=z0<=mesh.zmax-0.01 and -13<=y0<=13:
        ...
        for k in range(int(mesh.nz-1)):  #軌道から電場
            ...
            for j in range(sample):
                if len(zlist[j]) != 0 and getNearestValue(zlist, ylist, mesh.zmin+k*mesh.dz, j) < 13  :
