...
class CartesianGrid:                                                            #基本構造
    ...
    def make_cylinder(self, a, b, c, d, e, z1, z2, z3, z4, z5):
        Z_new = e
...
def solve_eq():
    ...
    mesh = CartesianGrid()
    a = int(input("電極間距離a："))
    V_init = int(input("印加電圧v："))

    # extrode boundary condition
    A = mesh.set_boundary_circle(V_init, 5, 19)
    B = mesh.set_boundary_circle(V_init, 17.8, 19)
    C = mesh.set_boundary_2circle(1e-100, V_init, 5, 12.7, 17.8, 19)
    D = mesh.set_boundary_2circle(1e-100, V_init, 11.5, 12.7, 17.8, 19)
    E = mesh.set_boundary_circle(V_init, 0, 19)
    electrode = mesh.make_cylinder(A, B, C, D, E, -1, 0, 2, 2+a, 4+a)

    A = calc_jacobi_matrix(mesh, electrode)

    k = mesh.convert_to_1d_array(electrode)

    iter_control = IterationControl(20000, 100, 5e-2)
    ...
    with open('Electrode/electrode_data/electrode_rad0.0_a{}_V{}.binaryfile'.format(a, V_init), 'wb') as lens:
        pickle.dump(k, lens)
